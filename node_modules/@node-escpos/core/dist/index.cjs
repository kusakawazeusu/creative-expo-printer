...
const getPixels__default = /*#__PURE__*/_interopDefaultLegacy(getPixels);
const iconv__default = /*#__PURE__*/_interopDefaultLegacy(iconv);
const qr__default = /*#__PURE__*/_interopDefaultLegacy(qr);
...
class Image {
  ...
  static load(url, type = null) {
    return new Promise((resolve, reject) => {
      getPixels__default(url, type ?? "", (error, pixels) => {
        if (error) {
          reject(error);
        } else {
...
class Printer extends EventEmitter__default {
  ...
  text(content, encoding = this.encoding) {
    return this.print(iconv__default.encode(`${content}${EOL}`, encoding));
  }
  drawLine(character = "-") {
    let buffer;
  ...
  table(data, encoding = this.encoding) {
    ...
    this.buffer.write(iconv__default.encode(lineTxt + EOL, encoding));
    return this;
  }
  tableCustom(data, options = { size: [1, 1], encoding: this.encoding }) {
    ...
    this.buffer.write(
      iconv__default.encode(lineStr + EOL, options.encoding || this.encoding)
    );
  ...
  pureText(content, encoding = this.encoding) {
    return this.print(iconv__default.encode(content, encoding));
  }
  encode(encoding) {
  ...
  qrcode(content, version, level, size) {
    if (this._model !== "qsprinter") {
      ...
      const dataRaw = iconv__default.encode(content, "utf8");
  ...
  qrimage(text, options = { type: "png", mode: "dhdw" }) {
    return new Promise((resolve, reject) => {
      const buffer = qr__default.imageSync(text, options);
      const type = ["image", options.type].join("/");
      getPixels__default(buffer, type, (err, pixels) => {
        if (err)
          reject(err);
